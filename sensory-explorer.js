...
let trashItems = []; 
let clouds = []; let raindrops = []; let flowers = []; let insects = []; let scene3Timer = 0;

// Level 4 (Music) Objects
...
const textDict = {
  EN: {
    ...
    lvl1: "Level 1: Wave your arms normally to clear the smog.",
    lvl2: "Level 2: Grab the pollution to clean the river.",
    lvl3: "Level 3: Touch clouds to make rain. Grow the garden!",
    lvl4: "Level 4: BRONX BEATS! Hit the instruments to make music!",
    lvl5: "Level 5: Pop Coral bubbles! Bounce the rest gently.",
  ...
  ES: {
    ...
    lvl3: "Nivel 3: Toca las nubes para llover. ¡Crece el jardín!",
    lvl4: "Nivel 4: ¡RITMOS! ¡Toca los instrumentos para la música!",
...
function draw() {
  ...
  if (fadeState > 0) {
    ...
    if (fadeState === 1) {
      ...
      fadeAlpha -= 4; if (fadeAlpha <= 0) { fadeAlpha = 0; fadeState = 0; }
    }
  }
}

function triggerTransition(targetScene) { if (fadeState === 0) { fadeState = 1; nextScene = targetScene; } }
...
function initScene3() { 
  clouds = [ // Massive overlapping canopy
    { x: width*0.15, y: 80, w: 400, h: 180 }, { x: width*0.5, y: 60, w: 500, h: 220 }, { x: width*0.85, y: 80, w: 400, h: 180 }
  ]; 
  raindrops = []; flowers = []; insects = []; scene3Timer = 1800; // ~30 secs freeplay
}
function initScene4() {
  instruments = [
...
function drawScene1() {
  if (fadeState === 0) { 
    // ERGONOMIC FIX: Combine Pointers and Bouncers so ANY hand/arm part wipes!
    let allErasers = activePointers.concat(activeBouncers);
    smogGraphics.erase();
    for (let pt of allErasers) {
      smogGraphics.circle(pt.x, pt.y, 160); // Huge forgiving brush
      smogCleared += 0.5;
    }
    smogGraphics.noErase();
  }
  image(smogGraphics, 0, 0);
  if (smogCleared > 1200 && fadeState === 0) triggerTransition(2);
}
...
// LEVEL 3: GARDEN (1:1 Growth & Distinct Flower Types)
function drawScene3() {
      ...
      if (r.y > height - 100) { 
        ...
        for (let f of flowers) {
          if (dist(f.x, 0, r.x, 0) < 50 && f.size < f.maxSize) {
            f.size += 5; // Raindrop adds 5 pixels of height!
            watered = true; break;
          }
        }
        if (!watered && flowers.length < 25) {
          // Type 0: Tulip (🦋), Type 1: Sunflower (🐝), Type 2: Daisy (✨)
  ...
  for (let f of flowers) {
    ...
    if (f.size >= f.maxSize) {
      ...
      if (fTouched && f.cooldown <= 0) {
        ...
        else spawnExplosion(f.x, fCenterY, [255, 215, 0]); // Golden pollen burst
        f.cooldown = 150; 
      }
    }
    if (f.cooldown > 0) f.cooldown--;
    
    // Draw Distinct Flowers
    fill(46, 204, 113, 200); rect(f.x - 3, fCenterY, 6, f.size); // Stem
    noStroke();
    if (f.type === 0) { // Tulip
      fill(255, 105, 180, 220); ellipse(f.x, fCenterY - 10, f.size/1.5, f.size);
    } else if (f.type === 1) { // Sunflower
      fill(255, 215, 0, 220); 
...
// LEVEL 4: BRONX BEATS (TWILIGHT MUSIC)
function drawScene4() {
  background(40, 20, 80, 80); // Twilight purple overlay
  ...
  for (let inst of instruments) {
    ...
    if (isHit && frameCount % 6 === 0) {
      ...
      inst.radius = 90; // Pop visually
    } else {
      if (inst.radius > 80) inst.radius -= 1; // Shrink back
    }
  }
  ...
  if (musicEnergy >= 1500 && fadeState === 0) triggerTransition(5);
}
...
function drawScene5() {
  ...
  if (bubblesPopped >= 30 && fadeState === 0) triggerTransition(6);
}
