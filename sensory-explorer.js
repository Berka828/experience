...
let activePointers = []; 
let activeBouncers = []; 
let activeFeet = []; // Specific array for stepping logic

// BxCM Brand Colors: Teal, Navy, Gold, Coral
...
const textDict = {
  EN: {
    ...
    lvl4: "Level 4: Pop the Coral bubbles! Bounce the rest gently.",
  ...
  ES: {
    ...
    lvl4: "Nivel 4: ¡Explota las corales! Rebota las demás suavemente.",
...
function draw() {
  ...
  if (currentScene === 1) drawScene1();
    ...
    if (frameCount % 720 === 0 && fadeState === 0) triggerTransition(1); 
  }
  ...
  if (fadeState > 0) {
    noStroke(); fill(255, 255, 255, fadeAlpha); rect(0, 0, width, height);
    if (fadeState === 1) {
      fadeAlpha += 4; 
      if (fadeAlpha >= 255) {
      ...
      fadeAlpha -= 4; 
      if (fadeAlpha <= 0) { fadeAlpha = 0; fadeState = 0; }
    }
  }
}
...
async function fetchLiveWeather() {
  try {
    ...
    let weatherCode = data.current_weather.weathercode; let isDay = data.current_weather.is_day;
    if (isDay === 0) skyColor = [20, 24, 82]; 
    else if (weatherCode >= 50 && weatherCode <= 67) { skyColor = [150, 160, 170]; isRainingInBronx = true; } 
...
function drawSkeletonsAndInteractions() {
  ...
  // STEP 1: Process Bodies (Provides the master coloring IDs)
  for (let i = 0; i < poses.length; i++) {
    let pose = poses[i].pose;
    let pColor = playerColors[i % playerColors.length];

    // Collect Body Hitboxes
    ...
    // Collect Feet specifically for stepping logic
  ...
  // STEP 2: Process Hands & Pair to Bodies
  for (let k = 0; k < trackedHandsData.length; k++) {
    ...
    // Pairing: Which PoseNet body wrist is this MediaPipe hand closest to?
    ...
    for (let i = 0; i < poses.length; i++) {
      ...
      let dL = dist(wrist[0], wrist[1], lw.x, lw.y);
      let dR = dist(wrist[0], wrist[1], rw.x, rw.y);
...
function initScene4() { popBubbles = []; bubblesPopped = 0; ripples = []; fireflies = []; for(let i=0; i<8; i++) spawnBubble(); for(let i=0; i<30; i++) fireflies.push({x: random(width), y: random(height), vx: random(-0.5, 0.5), vy: random(-0.5, 0.5)}); }

function spawnBubble() { 
  let baseSize = parseInt(safeGetValue('bubble-size', 45));
  let isRed = random() < 0.4; // 40% chance it's Coral Red
  
  // Pick a random BxCM brand color, unless it is forced Red
  let bColor = isRed ? bxcmColors[3] : random([bxcmColors[0], bxcmColors[1], bxcmColors[2]]);
  
  popBubbles.push({ 
    x: random(100, width-100), y: -100, // Spawn from TOP
    vx: random(-1, 1), vy: random(2, 4), // Float DOWN
...
function drawScene1() {
  if (handVelocity > 5 && fadeState === 0) { 
    ...
    for (let k = 0; k < activePointers.length; k++) {
      smogGraphics.circle(activePointers[k].x, activePointers[k].y, 100); smogCleared += 0.5;
    }
...
function drawScene3() {
  ...
  for (let c of clouds) {
    fill(220, 220, 220, 200); noStroke(); ellipse(c.x, c.y, c.w, c.h); ellipse(c.x-30, c.y+10, c.w*.7, c.h*.8); ellipse(c.x+30, c.y+10, c.w*.7, c.h*.8);
    let isTouched = false;
...
function drawScene4() {
  ...
  // SUN INTERACTION: Touch to spawn sun particles
  ...
  if (sunTouched && frameCount % 4 === 0) {
    sunSize += 20; // Visual bump
    spawnExplosion(sunX, sunY + 80, [255, 255, 0]);
  }
  ...
  for (let i = popBubbles.length - 1; i >= 0; i--) {
    ...
    if (b.active) {
      ...
      // BOUNCE OFF PALMS & FEET
      for (let kicker of activeBouncers) {
        if (dist(kicker.x, kicker.y, b.x, b.y) < b.radius + 40) {
          let angle = atan2(b.y - kicker.y, b.x - kicker.x);
          b.vx = cos(angle) * 7; b.vy = sin(angle) * 7 - 2; // Bounce UP and Away
          ripples.push({x: b.x, y: b.y, radius: b.radius, alpha: 200});
        }
      }

      // POP MECHANICS (Pointer Finger or Stepped on)
      let pointed = false;
      for (let pointer of activePointers) { if (dist(pointer.x, pointer.y, b.x, b.y) < b.radius) pointed = true; }
      
      let steppedOn = false;
      if (b.y > height - b.radius) {
        // If it's on the ground, check if a foot is near it
      ...
      if (pointed || steppedOn) {
        if (b.isRed) {
          ...
          b.vy = -3; // Pastels float up if poked
        }
      } else if (b.y > height - b.radius) {
        // If it hits the floor and isn't stepped on, it bounces back up
        b.vy = -6; 
      }
