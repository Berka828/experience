...
function setup() {
  ...
  video = createCapture(VIDEO, function(stream) { 
    ...
    video.elt.muted = true; // Crucial for browser autoplay policies
...
class Particle {
  constructor(x, y, colorArray) {
    ...
    this.life = 255; this.color = colorArray; 
  }
...
function spawnExplosion(x, y, colorArray) { 
  for (let i = 0; i < 20; i++) particles.push(new Particle(x, y, colorArray)); 
}
...
function drawSkeletonsAndInteractions() {
  ...
  for (let k = 0; k < trackedHandsData.length; k++) {
    ...
    if (isGrabbing) {
      ...
      // Fallback: If hands are low, they act as feet for stomping bubbles
